...
import { emptyAssessment, loadAssessment, saveAssessment } from "@/lib/storage";
...
export default function ResultsPage() {
  ...
  return <><SiteHeader /><main className="results-main"><section className="report-header"><div className="shell"><div className="report-kicker"><span>PAIR Readiness Profile</span><span>Preliminary evidence-based assessment</span></div><div className="report-title-row"><div><h1>{state.meta.caseName || "Draft readiness profile"}</h1><p>{state.meta.organization} · {state.meta.geography} · {state.meta.useCase}</p><small>Assessor: {state.meta.assessorLabel} · Assessment date: {state.meta.assessmentDate} · Framework v{state.frameworkVersion}</small></div><div className="report-actions"><button onClick={() => window.print()}>Print / Save as PDF</button><button onClick={exportProfile}>Export PAIR JSON</button><button onClick={exportSprintBrief}>Create Readiness Sprint Brief</button></div></div>{message && <p className="report-status" role="status">{message}</p>}</div></section>
    <div className="shell report-body">{!completion.readyForProfile && <section className="draft-notice"><h2>A working draft</h2><p>You can explore and share this profile before completing the assessment. Unanswered dimensions remain unknown. Only supported ratings contribute to summary measures.</p><a href="/assessment">Continue the assessment</a></section>}{state.automatedEvidence?.packages.length ? <PublicEvidenceWorkbench state={state} onChange={(next)=>{setState(next);saveAssessment(next);}} /> : null}
